...
def predict(state, kernel, x_i):
    ...
    # *** START CODE HERE ***
    sum_kernel = 0;
    for beta_prev, x_prev in state:
        sum_kernel += beta_prev * kernel(x_prev, x_i)
    return sign(sum_kernel)
    # *** END CODE HERE ***
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    """
    # *** START CODE HERE ***
    curr_beta = learning_rate * (y_i - predict(state, kernel, x_i))
    state.append((curr_beta, x_i))
    # *** END CODE HERE ***
