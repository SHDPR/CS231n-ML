...
def predict_from_naive_bayes_model(model, matrix):
    ...
    p_y1 = (log_phi_k_y1 * matrix).sum(axis=1) + np.log(phi_y)
    p_y0 = (log_phi_k_y0 * matrix).sum(axis=1) + np.log(1 - phi_y)
...
def get_top_five_naive_bayes_words(model, dictionary):
    ...
    # *** START CODE HERE ***
    phi_k_y1, phi_k_y0, phi_y = model
    word_index = (np.log(phi_k_y1) - np.log(phi_k_y0)).argsort()[-5:]
    dict_key = list(dictionary.keys())
    return [dict_key[index] for index in word_index]
    # *** END CODE HERE ***
